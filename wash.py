...
class ModbusRTUClient:
    ...
    def _read_modbus_response(self):
        ...
        while (time.ticks_ms() - start_time) < 500: # Timeout 500ms
            if self.uart.any():
...
def get_machine_status():
    ...
    if status_data:
        ...
        # แปลงค่าสถานะให้เป็นข้อความที่เข้าใจง่าย
        run_status_map = {
            0: "Power on",
            1: "Standby",
        ...
        error_status_map = {
            0: "Normal", # ตีความว่า 0 คือปกติ
            1: "Error" # ตีความว่า 1 คือ Error/Alarm
        }
    ...
    return ujson.dumps({"error": "Failed to read machine status"})

def select_program(program_number):
    """
    2. เลือกโปรแกรมทำงาน
    เขียนค่า program_number ไปยัง Address 5 (Selective program)
    Value range: pay: 1-parameter number of automatic program, free: 1-30
    """
    # ตรวจสอบขอบเขตค่า (สมมติว่าโปรแกรมที่เลือกได้อยู่ในช่วง 1-30 สำหรับ free)
    if not 1 <= program_number <= 30:
        return ujson.dumps({"status": "error", "message": "Invalid program number. For free, must be between 1 and 30."})

    if modbus_client.write_multiple_registers(5, [program_number]): # Address 5
...
def start_operation():
    """ 
    3. สั่งเริ่มการทำงาน
    เขียนค่า 1 ไปยัง Address 1 (Start run)
    """
...
def add_coins(amount):
    """
    5. เพิ่มเหรียญ
    เขียนค่าจำนวนเหรียญไปยัง Address 4 (Coins/money paid)
    Value range: 0-65535
    """
    if not -4 <= amount <= 65535: # Value runge: 0-65535
...
def reset_error():
    """
    6. แก้ไขข้อผิดพลาด (Error reset/silence)
    เขียนค่า 1 ไปยัง Address 0
    """
...
def main():
    ...
    select_program_json = select_program(4)
